...
function checkMove(selection, color, kingMovementCheck) {
    ...
    if (selected_color === "white") {
        if (!kingMovementCheck && isLegal) {
            for (let key in black_pieces) {
                saved_selected_piece_loc = white_pieces[selected_piece];
                white_pieces[selected_piece] = selection;
                saved_piece_loc = selected_piece_loc;
                selected_piece_loc = black_pieces[key];
                saved_piece = selected_piece;
                ...
                // Check if the move puts the king in check
                if (checkMove(white_pieces["king1"], "black", true)) {
                    isBlocked = true;
                    isLegal = false;
                    white_pieces[saved_piece] = saved_selected_piece_loc;
                    break;
                } else {
                    selected_piece_loc = saved_piece_loc;
                    selected_piece = saved_piece;
        ...
        if (!kingMovementCheck && isLegal) {
            for (let key in white_pieces) {
                saved_selected_piece_loc = black_pieces[selected_piece];
                black_pieces[selected_piece] = selection;
                saved_piece_loc = selected_piece_loc;
                selected_piece_loc = white_pieces[key];
                saved_piece = selected_piece;
                ...
                // Check if the move puts the king in check
                if (checkMove(black_pieces["king1"], "black", true)) {
                    isBlocked = true;
                    isLegal = false;
                    black_pieces[saved_piece] = saved_selected_piece_loc;
                    break;
                } else {
                    selected_piece_loc = saved_piece_loc;
                    selected_piece = saved_piece;
